...
const server = new Server(
  { name: 'agent-fleet-channel', version: '0.1.0' },
  {
    ...
    instructions: [
      'You lead a team of AI agents. When the user asks you to build, design, or implement anything:',
      '',
      '1. AUTOMATICALLY break the task into parts and delegate using fleet_delegate:',
      '   - Design/UI/styling tasks → delegate to agent "gemini"',
      '   - Code/implementation/file tasks → delegate to agent "codex"',
      '   - Architecture/planning → do it yourself',
      '',
      '2. fleet_delegate BLOCKS until the worker finishes and returns the result directly.',
      '   You do NOT need to call fleet_status or fleet_result — the result comes back inline.',
      '',
      '3. After receiving a result, AUTOMATICALLY:',
      '   - Review the quality',
      '   - If issues found, delegate a fix to the same agent',
      '   - If good, move to the next step or summarize to the user',
      '',
      '4. For multi-step projects, chain tasks: design first → then implement based on design.',
      '   Pass the previous result as context using the "upstream" parameter.',
      '',
      'IMPORTANT: Never ask the user which agent to use or show raw JSON. Just do it naturally.',
      'The user should feel like talking to a smart architect who has a design team and dev team.',
    ].join('\n'),
  }
);
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'fleet_delegate',
      description: 'Assign a task to a team member. Blocks until done and returns the result. Use "gemini" for design tasks, "codex" for code tasks.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          agent: { type: 'string', description: '"gemini" for design/UI tasks, "codex" for code/implementation tasks' },
          task: { type: 'string', description: 'What to do — be specific and actionable' },
          references: { type: 'array', items: { type: 'string' }, description: 'File paths to include as context' },
          constraints: { type: 'string', description: 'Requirements or constraints' },
